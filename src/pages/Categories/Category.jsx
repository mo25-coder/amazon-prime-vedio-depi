...
function Category() {
  ...
  const [checkList, setCheckList] = useState([
    { category: "movies", checked: false },
    { category: "series", checked: false },
  ]);
  const movies = categoriesArray.filter(
    (cat) => cat.category.toLowerCase() === categoryType.toLowerCase()
  )[0].movies;

  let filteredMovies =
    checkList.every((ch) => ch.checked === true) ||
    checkList.every((ch) => ch.checked === false)
      ? movies
      : movies.slice(0, movies.length / 2);
